...
class Website:
    ...
    def __init__(self, url: str, check_network:bool = False) -> None:
        ...
        self.url = url
        global logger
        logger = WidgetLogger(url = url)
        logger.info(f"New website instance: {url}")
        ...
        try:
            ...
            try:
                if len(self.soup.meta['content']) >0:
                    logger.info(f"Redirection to -> {self.url}")
            ...
            if len(self.md)>0 :
                logger.info(f"{WidgetStatus.WIDGET_FOUND.value}")
                ...
                logger.info(f"OUTCOME is: {self.outcome}")
                ...
                for ww in self.all_widget:
                    ...
                    if ww[0]['href'] not in list(REMOVED_LINKS):
                        ...
                        if type(page) is Website:
                            ...
                            logger.info(f'FOUND WIDGET ON: {self.outcome.page_found}')
            ...
            logger.error("XML file", exc_info = True)
            print(e6)
            self.outcome = CheckWidget(url = self.url,outcome = WidgetStatus.CONNECTION_ERROR.value)
        except MarkupResemblesLocatorWarning as e7:
            logger.error("CSS file", exc_info = True)
            print(e7)
            self.outcome = CheckWidget(url = self.url,outcome = WidgetStatus.CONNECTION_ERROR.value)
    ...
    #             logger.info(f"FOUND WIDGET in: {link}")
    #             print('BREAKING')
    ...
    def check_MD(self, url_md = None):
        try:
            ...
            if len(self.md)>0 and 'strutture' in self.md[0]['href']:
                ...
                logger.info(f"Facility widget - Mapping: {self.outcome.allow_booking}")
            ...
            if len(self.md)>0 and 'strutture' not in self.md[0]['href']:
                ...
                logger.info(f"individual widget - Mapping: {self.outcome.allow_booking}")
    ...
    def handle_redirection(self)-> str:
        try:
            ...
            logger.info(f"Redirectig to {redir}")
            ...
            logger.error("Redirection Error", exc_info = True)
